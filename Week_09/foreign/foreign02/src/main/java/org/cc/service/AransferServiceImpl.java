...
@DubboService(interfaceName = "aTransferService")
public class AransferServiceImpl implements ATransferService {
    @Autowired
    ...
    public void transfer() {
        //1.人民币；2美元
        //系统账户
        Account totalAccount = jdbcTemplate.queryForObject("SELECT * FROM t_account WHERE userid = ? AND currency = ?", (ResultSet rs, int arg1) -> toAccount(rs)
                , 0, 2);
        //用户账户
        Account account = jdbcTemplate.queryForObject("SELECT * FROM t_account WHERE userid = ? AND currency = ?", (ResultSet rs, int arg1) -> toAccount(rs)
                , 1, 1);
        Account dollarAccount = jdbcTemplate.queryForObject("SELECT * FROM t_account WHERE userid = ? AND currency = ?", (ResultSet rs, int arg1) -> toAccount(rs), 1, 2);
        //冻结1美元
